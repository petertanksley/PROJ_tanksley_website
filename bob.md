...
effort_remaining: ~1h (Nature DOI swap; copy review). Hex stickers deferred.
...
- [x] Peter reviewed research.qmd copy (2026-08-28): intro rewritten, GEB cover restored, humanizer pass, Research Ring naming, featured articles swapped (JOEM out; Martaindale & Tanksley JCJ in)
- [x] Deleted unused styles.css (approved 2026-08-28)

## Side project: hex stickers (deferred 2026-08-28)

**Plan (when picked up):** replicate Hadley Wickham's slide-illustration pipeline
(https://tidydesign.substack.com/p/illustrating-my-slides-with-ai) using his R package
`bananarama` (`pak::pak("hadley/bananarama")`): YAML with a shared `style` prompt + per-image
`description`, `aspect-ratio: "1:1"`, `n: 3` variants, `seed`, `[refimage]` references for
consistency. Gemini image model (~$0.07/image) via `ellmer::chat_google_gemini()`. Then wrap
the PNG in R with `magick`: clip to point-up hex, ink border, title. Model paints, code frames.
**Blocker:** needs a `GEMINI_API_KEY` (Google AI Studio) or Vertex credentials. TXST Gemini
access is almost certainly the Workspace app, not an API key. Peter plans a personal AI Studio
account + payment method. Put the key in `~/.Renviron`, never in the repo.
**Also missing locally:** `pak`, `ellmer` (magick, yaml, httr2 present).

History:

- Peter wants point-up hex stickers (like package hex stickers, as on alerrt-research.org) as section marks. Prototype (Manhattan plot + police cap) shelved 2026-08-28: cap not recognizable; Peter is looking for a different approach. Files deleted 2026-08-28 at Peter's request. Cartoon Manhattan plot with a police cap on the y-axis, site palette, generated by `www/hex/make_hex.R` (ggplot2 + svglite, seeded). R-only by Peter's standing rule.
