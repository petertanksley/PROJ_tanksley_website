...
- Peter wants point-up hex stickers (like package hex stickers, as on alerrt-research.org) as section marks. Prototype shipped: `www/hex/origins.svg` (full, with text) + `origins-icon.svg` (text-free, used beside the "Where I came from" h2). Cartoon Manhattan plot, site palette, generated by a small Python script (seeded).
- If Peter likes it: one sticker per Research section (health/mortality: survival curve or Lexis surface; training/opinion: factorial-survey grid or a stick-figure use-of-force vignette), one for Projects, one for Left-hand Thoughts (spiral notebook). Move the generator into `www/hex/make_hex.py` so they share geometry and palette.
- Claude draws SVG by hand; no raster image generation. For painterly/photographic stickers Peter would need an external image tool, then we clip to the hex.
