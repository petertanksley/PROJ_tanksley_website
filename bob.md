...
- Peter wants point-up hex stickers (like package hex stickers, as on alerrt-research.org) as section marks. Prototype shipped: `www/hex/origins.svg` (full, with text) + `origins-icon.svg` (text-free, used beside the "Where I came from" h2). Cartoon Manhattan plot with a police cap on the y-axis, site palette, generated by `www/hex/make_hex.R` (ggplot2 + svglite, seeded). R-only by Peter's standing rule.
